...
class UNet_none(torch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, feature_list=None):
        ...
        for i in range(len(feature_list) - 1):
            conv_layer = []
            conv_layer.append(Conv_Block_UNet(
                feature_list[i], feature_list[i + 1], 3, 1, 1, norm=False))
            conv_layer.append(Conv_Block_UNet(
                feature_list[i + 1], feature_list[i + 1], 3, 1, 1, norm=False))
            encoder.append(torch.nn.Sequential(*conv_layer))
        self.encoder = torch.nn.Sequential(*encoder)
...
class Deeper_UNet(torch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, feature_list=None, hcr=False, output1_ch=6, output2_ch=12, attention=False):
        ...
        self.last_conv = Conv_Block_UNet(
            decode_feature_list[-1], feature_list[0], 3, 1, 1)
        if hcr is True:
            self.output_6 = Conv_Block_UNet(
                feature_list[0], output1_ch, 3, 1, 1, norm=False)
            self.output_output1_ch_cat = Conv_Block_UNet(
                feature_list[0] + output1_ch, feature_list[0], 3, 1, 1, norm=False)
            self.output_12 = Conv_Block_UNet(
                feature_list[0], output2_ch, 3, 1, 1, norm=False)
            self.output_output2_ch_cat = Conv_Block_UNet(
                feature_list[0] + output2_ch, feature_list[0], 3, 1, 1, norm=False)
        self.output = torch.nn.Conv2d(feature_list[0], output_ch, 1, 1, 0)
